...
func New(cfg *config.Config, jwtSvc *auth.JWTService, authHandler *auth.Handler, passkeyHandler *auth.PasskeyHandler) *Server {
	s := &Server{cfg: cfg, authHandler: authHandler, passkeyHandler: passkeyHandler}
	s.router = s.buildRouter(jwtSvc)
	return s
}
...
func (s *Server) buildRouter(jwtSvc *auth.JWTService) chi.Router {
	r := chi.NewRouter()
