...
func (r *PostgresPasskeyRepository) Create(ctx context.Context, cred PasskeyCredential) error {
	...
	_, err := r.pool.Exec(ctx, q, cred.UserID, cred.CredentialID, cred.PublicKey, cred.SignCount)
	if err != nil {
...
func (r *PostgresPasskeyRepository) UpdateSignCount(ctx context.Context, credID string, count uint32) error {
	const q = `UPDATE passkey_credentials SET sign_count = $1, last_used_at = NOW() WHERE credential_id = $2`

	_, err := r.pool.Exec(ctx, q, count, credID)
	if err != nil {
